...
void MetRK(double y[m], double x, double xN, double h, double eps)
{
	...
	cout << setw(10) << "x" << setw(10) << "y[0]" << setw(10) //заголовки таблицы
		...
		setw(10) << "el" << endl;
	cout << fixed << setw(10) << setprecision(4) << x << setw(10) << y[0] << setw(10) << y[1] << setw(10) <<
		y[2] << setw(10) << y[3] << setw(10) << h << setw(10) << el << endl;
	for (;;)
	{
...
int main() {
	...
	double x0 = 0, xN = 1, h = 0.05; // начальные значения и шаг
	cout << "\th=" << h << endl;
	double eps = 0.1;//h; //погрешность
