...
public:
	...
	void Create(Matrix B, int k)
	{
		for (int i = 0; i < size; i++)
		{
			(*this)[i] = B.ReturnElementMatrix(i,k-1);
		}
	}
...
bool Check(Matrix A, Vector x, Vector b, int size) {
	...
	for (int i = 0; i<size; ++i)
	{
		...
		{
			flag[0] = 1; cout << "Погрешность: " << abs(Result[i] - b[i]) << endl;
		}
		else
...
int main()
{
	...
	cout << "Матрица создается рандомная."<<endl;
	...
	int ch = 0;
	int k = 0;
	cout << "Как хотите создать вектор свободных членов?\nВедите 1 чтобы создать рандомный,\nВведите 2 чтобы создать из любого столбца матрицы А\n";
	cout << "choose="; cin >> ch;
	if (ch == 1)
		b.Random();
	else if (ch == 2)
	{
		cout << "Какой стоблец исходной матрицы хотите выбрать\nдля создания вектора свободных членов?: ";
		cin >> k;
		if (k <= 0 || k>size)
		{
