...
double sum1 = 0;
double sum2 = 0;
((*this)[0][0]>0) ? D[0][0] = 1 : D[0][0] = -1; //1
S[0][0] = sqrt(abs((*this)[0][0]));
for (int j = 1; j < size; ++j)
S[0][j] = (*this)[0][j] / (S[0][0] * D[0][0]);
...
(((*this)[i][i] - sum1)>0) ? D[i][i] = 1 : D[i][i] = -1; //2
S[i][i] = sqrt(abs((*this)[i][i] - sum1));
}
else if (i+1<=j)
{
sum2 = 0;
...
S[i][j] = ((*this)[i][j] - sum2) / (S[i][i] * D[i][i]);
}
else
S[i][j] = 0;
sum1 = 0;
...
R = T.MultMatrix(S);
cout << "\nMatrix A:" << endl;
(*this).showData(); //symmetric matrix
cout << "\nSt*D*S" << endl;
R.showData();
